...
{
	// m = rows, n = cols, nnz = non-zero elements
	// row_ind, col_ind = indexes for rows and cols in COO 
	// val = value of each non-zero element in COO 

	// HAS TO BE O(N) 
	
	// Allocating memory
	// m + 1 because of extra last pointer
	...
	*csr_vals = (double *) malloc(sizeof(double) * nnz);
	...
	unsigned int *histogram = malloc(sizeof(unsigned int) * (m + 1));
	for (int i = 0; i <= m; i++) {
		histogram[i] = 0;
	}
	
	// Histogram count to get offset
	for (int i = 0; i < nnz; i ++) {
		histogram[row_ind[i]]++;
	}
	...
	// Convert to CSR
	for (int i = 0; i < nnz; i++) {
		unsigned int count = (*csr_row_ptr)[row_ind[i]]++;
		(*csr_col_ind)[count] = col_ind[i];
		(*csr_vals)[count] = val[i];
...
{
	// Loop over m rows and res array to 0
	for (int i = 0; i < m; i++) {
		res[i] = 0.0;
