...
namespace Mem {
  ...
  public:
    ...
    // Grabbing the head
    Elem<int>* grabHead() const { return head;}
    ...
    // Friend statement to access IntQueue 
    friend std::ostream& operator<<(std::ostream &os, const IntQueue& queue);
...
//
void Mem::IntQueue::Enqueue(int newValue)
{
...
std::ostream& operator<<(std::ostream& os, const Mem::IntQueue& queue) {
	// Get head of queue
	Mem::Elem<int>* curr = queue.grabHead();
	
	// If the queue isn't NULL get the data and go next 
	while (curr != NULL) {
		os << curr->getData();
		curr = curr->getNext();
	}
...
int main()
{
  ...
  // Dequeue all the items in queu
  int value;
  while (Q.grabHead() != NULL) {
	// Dequeue and print 
	Q >> value;
	std::cout << value << " ";	
  }
